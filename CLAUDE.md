...
- **No inline templates or styles.** Every component's HTML and CSS live in their own files, referenced with `templateUrl` / `styleUrl` — never `template:` or `styles:` in the decorator, however short the markup. Stated by the owner on 2026-07-30. Markup in a decorator loses editor tooling, makes diffs noisy, and grows into an unreadable string exactly where the UI is most worth reading.
- **A deferral is only real once it is an unsatisfied requirement in the system of record.** Writing "roadmap, not v0" into a markdown file leaves the doneness gate blind to it — that is how the whole operator console vanished from the plan while all five pillars reported green (defect `cd18baa0`). If it is deferred, it is a requirement in Tasktracker with unsatisfied acceptance criteria.
